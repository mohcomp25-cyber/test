...
المزامنة تعمل تلقائياً كل يوم **الساعة ١٢ ليلاً بتوقيت الرياض** (قابلة للتغيير عبر `REVIEWS_CRON`)، وتسحب **مراجعات يوم العمل فقط** (من أمس بتوقيت الرياض، بحد أقصى `APIFY_MAX_REVIEWS`=50) توفيراً لاستهلاك توكن Apify — والتكرار يُرشَّح تلقائياً. يمكن تشغيلها يدوياً بزر «مزامنة الآن» من أي صفحة. التصنيف: ٤+ إيجابي، ٣ محايد، ٢ وأقل سلبي.
