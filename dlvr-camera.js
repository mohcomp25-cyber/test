...
async function startCam(deviceId){
  ...
  // لو ما اشتغلت — ابحث بذكاء عن USB
  if(!stream){
    try{
      // اطلب إذن أول عشان نقدر نشوف أسماء الكاميرات
      ...
      console.log('[DLVR] الكاميرات المتاحة:');
      cams.forEach((c,i)=>console.log(`  [${i}] "${c.label}" id=${c.deviceId.slice(0,12)}...`));

      // === استراتيجية البحث عن USB ===

      // 1) أولوية عالية: كاميرا فيها كلمة USB صريحة
      let usbCam=cams.find(c=>isLikelyUSB(c.label));

      // 2) أولوية متوسطة: أي كاميرا ليست مدمجة في الجوال
      if(!usbCam && cams.length>1){
        usbCam=cams.find(c=>{
          const label=c.label||'';
          // لو ما فيه label نتخطاها
          if(!label)return false;
          // لو مو كاميرا مدمجة، غالباً USB
          return !isBuiltinCamera(label);
        });
      }

      // 3) أولوية منخفضة: لو فيه أكثر من 2 كاميرا، آخر وحدة غالباً USB
      //    (كاميرات الجوال عادة index 0 و 1)
      if(!usbCam && cams.length>2){
        usbCam=cams[cams.length-1];
      }

      if(usbCam){
        try{
          stream=await navigator.mediaDevices.getUserMedia({video:{deviceId:{exact:usbCam.deviceId},width:{ideal:1920},height:{ideal:1080}},audio:false});
          selectedCamId=usbCam.deviceId;
          localStorage.setItem(KEY_CAM,selectedCamId);
          toast('📷 '+( usbCam.label || 'كاميرا خارجية'),'ok');
          console.log('[DLVR] تم اختيار: "'+usbCam.label+'"');
        }catch{stream=null;}
      }

      // لو USB ما اشتغلت — جرّب كل الكاميرات وحدة وحدة (بدءاً من الأخيرة)
      if(!stream){
        const reversed=[...cams].reverse();
        for(const cam of reversed){
          try{
            ...
            console.log('[DLVR] fallback اختار: "'+cam.label+'"');
            break;
          }catch{continue;}
        }
      }
  ...
  // fallback أخير — لا نستخدم facingMode عشان ما يرجع لكاميرا الجوال
  if(!stream){
...
navigator.mediaDevices.addEventListener('devicechange',async()=>{
  ...
  // دوّر على كاميرا جديدة (USB أو خارجية)
  const externalCam=cams.find(c=>{
    const label=(c.label||'').toLowerCase();
    return isLikelyUSB(label) || (label && !isBuiltinCamera(label) && cams.length>2);
  });

  if(externalCam&&externalCam.deviceId!==selectedCamId){
    toast('📷 كاميرا جديدة! جاري التبديل...','info');
    setTimeout(()=>startCam(externalCam.deviceId),1000);
  }
});
